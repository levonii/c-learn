	...
	for (y = 1.3; y >= -1.1; y -= U)
	{
		for (x = -2; x < 1.4; x += V)
		{
			if ((((x * x + y * y - 1) * (x * x + y * y - 1) * (x * x + y * y - 1) - x * x * y * y * y) <= 0))
			{
				...
				{
					s++;
					if (s % 4 == 1) { SetColor(c, 0); printf("H"); }
					if (s % 4 == 2) { SetColor(c, 0); printf("L"); }
					if (s % 4 == 3) { SetColor(c, 0); printf("J"); }
					if (s % 4 == 0) { SetColor(0, 0); printf(" "); }
				}
				else
				{
					for (i = 0; i < 42; i++)
					{
						if (i <= 14 || i >= 28)
						{
							...
							if (s % 4 == 2) { SetColor(e, 0); printf("w"); }
							if (s % 4 == 3) { SetColor(c, 0); printf("b"); }
							if (s % 4 == 0) { SetColor(d, 0); printf(" "); }
						}
						else
						{
							SetColor(b, 0);
